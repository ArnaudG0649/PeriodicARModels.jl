...
function Monthly_stats(stats::Function, Monthly_Estimators, type_param=1)
    ...
    for i in 1:12
        ...
        append!(Output, length(L[1]) != 1 ? [stats.(invert_v(L))] : stats(L))
    end
    return Output
end
...
function AllEstimation(x::AbstractVector, p::Integer=1; Estimators::AbstractVector=[zeros(p); 1e-5], algo=LBFGS(), Date_vec=nothing, TrueParamVec=nothing, plot=true, meanparam=true, medianparam=true, ErrorTable=false, PerMethod=false, lineplot=false)
    ...
    Output = plot ? [ParamOutput, PlotParameters(isnothing(TrueParamVec) ? invert_v(ParamOutput) : invert_v([ParamOutput; [TrueParamVec]]), lineplot, !isnothing(Date_vec))] : [ParamOutput]
    ...
    if length(Output) == 1
        Output = Output[1]
        PerMethod ? nothing : Output = invert_v(Output) #that is to say Output[i][j] : i -> index of the parameter (Φᵢ) or (σ), j -> method 
        ErrorTable ? Output = [[Output]; TableError(Output, (medianparam && !meanparam) ? "median" : "mean")] : nothing
    else
        PerMethod ? nothing : Output[1] = invert_v(Output[1])
...
function TakeParameters(Parameters_vec::AbstractVector, method::String="mean")
    Parameters_vec = invert_v(Parameters_vec)
    if method == "mean"
    ...
    return invert_v(raw[1:end-1]), raw[end]
end
